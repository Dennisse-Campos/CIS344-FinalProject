...
BEGIN
    ...
    ELSE
        INSERT INTO Users(userName, contactInfo, userType, password)  -- Added password here
        VALUES (p_userName, p_contactInfo, p_userType, ''); -- added empty password, you should use a real one
    END IF;
END //
DELIMITER ;
...
BEGIN
    ...
    DECLARE v_pricePerPill DECIMAL(10,2);  -- Declare variable to store price per pill
    DECLARE v_saleAmount DECIMAL(10,2);    -- Declare variable for the sale amount
    
    -- Get medicationID and prescription quantity --  
    SELECT medicationId, quantity 
    INTO v_medicationId, v_prescriptionQuantity
    FROM Prescriptions
    ...
    IF v_quantityAvailable >= p_quantitySold THEN
        ...
        SET v_saleAmount = p_quantitySold * v_pricePerPill;
        
        -- Insert sale record WITH saleAmount
